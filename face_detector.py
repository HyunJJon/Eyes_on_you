    ...
    def _postprocess(self, imgs, origimgs, pred, conf_thres, iou_thres):
        ...
        for i in range(len(origimgs)):
            ...
            for j in range(det.size()[0]):
                ...
                box = list(
                    map(
                        int, [box[0] * w, box[1] * h, box[2] * w, box[3] * h]
                    )
                )
    ...
    def predict(self, imgs, conf_thres=0.3, iou_thres=0.5):
        ...
        if one_by_one:
            ...
            for num, img in enumerate(images):
                ...
                bb, pt = self._postprocess(
                    img, [origimgs[num]], single_pred, conf_thres, iou_thres
                )
                # print(bb)
                bboxes[num] = bb[0]
                points[num] = pt[0]
