...
while True:
    ...
    for box in bboxes[0]:
        ...
        if mode == 1 and distance > 70:
            ...
            cv2.circle(
                frame, (target_center_x, target_center_y), 70, (0, 255, 0), 1
            )
            if skip_flag:
                skip_flag = False
                continue
        ...
        if mode == 2 and distance <= 90:
            ...
            cv2.circle(
                frame, (target_center_x, target_center_y), 10, (0, 255, 0), 1
            )
        elif mode == 2 and distance > 70:
            mode = 1
        print(
            f"Distance: {distance}, Direction_x: {direction_x}, Direction_y: {direction_y}), mode: {mode}"
        )
